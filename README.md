...
- **Coding exercises are the exception to "runnable from the guide alone."** For code, "fully runnable, verbatim, copy-paste and it works" is a nice-to-have, not a requirement, and it can actively work against the point of the exercise. Prefer skeleton code, fill-in-the-blank sections, or "here is the pattern, now apply it" over a complete solution a student could paste in without understanding it. A coding exercise's format in a homework handout, code blocks and filled-in patterns rather than the checkbox-table rendering used elsewhere in that section, is also expected to differ from the rest of the handout; that is not a defect to flag in review. All of this governs student-facing material. A weekly teacher guide may additionally carry a Section 14, "Complete working code," holding tested and complete implementations of that week's programs including the extensions. Teacher guides are never uploaded to students, and that section exists so the instructor has a known-good fallback when live coding goes wrong in front of the room. Complete code there is deliberate rather than a lapse of this convention, and a review pass should not reduce it to a skeleton.
...
- **A quality or difficulty review is scoped to that week's Extra Credit AP Track section for AP claims.** When reviewing or revising a week for pacing, difficulty, or format, AP alignment claims found anywhere else in the guide, Section 12's framework mapping, an in-class aside, a script in Section 7, are out of scope for that pass unless the review is specifically about AP alignment. Verify or adjust AP pointers only inside the Extra Credit AP Track section itself, so an unrelated review pass does not silently touch AP coverage claims it was not asked to check.
